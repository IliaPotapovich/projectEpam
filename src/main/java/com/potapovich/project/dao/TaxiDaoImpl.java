...
public class TaxiDaoImpl {
    ...
    public Taxi findActiveTaxiByDriverLoginAndPass(String login, String password) throws DaoException {

        Taxi taxi = new Taxi();
        int id = new TaxiDriverDaoImpl().findDriverIdByEnteredLoginAndPassword(login, password);

        try (Connection connection = ConnectionPool.getInstance().getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(Constant.FIND_ACTIVE_TAXI_BY_DRIVER_ID)) {
            ...
            if (id > 0) {
                while (rs.next()) {
                    ...
                    taxi.setTaxiCar(new TaxiCarDaoImpl().findCarById(rs.getInt(2)));
                    taxi.setTaxiDriver(new TaxiDriverDaoImpl().findDriverById(rs.getInt(3)));
                    taxi.setActive(rs.getBoolean(4));
                    taxi.setTaxiLocation(new Point(rs.getInt(5), rs.getInt(6)));
                    taxi.setStartWorkDate(rs.getString(7));
            ...
            throw new DaoException("findActiveTaxiByDriverLoginAndPassError ", e);
        }
        return taxi;
    }
    ...
    public List<Taxi> findListOfWorkingTaxi() throws DaoException {
        ...
        try (Connection connection = ConnectionPool.getInstance().getConnection();
            ...
            while (rs.next()) {
                ...
                taxi.setId(rs.getInt(1));
                if (taxi.getId() > 0) {
                    taxi.setTaxiDriver(new TaxiDriverDaoImpl().findDriverById(rs.getInt(2)));
                    taxi.setTaxiCar(new TaxiCarDaoImpl().findCarById(rs.getInt(3)));
                    taxi.setActive(rs.getBoolean(4));
                    taxi.setTaxiLocation(new Point(rs.getInt(5), rs.getInt(6)));
                    taxi.setStartWorkDate(rs.getString(7));
                    listOfTaxi.add(taxi);
    ...
    public void changeTaxiFreeTrue(int taxiId) throws DaoException {
        ...
        String inserted = Constant.CHANGE_TAXI_FREE_STATUS_TRUE;
