...
<c:set var="langPage" scope="session" value="/jsp/user/trip_calculation.jsp"/>
        ...
        <nav class="navbar navbar-light" id= "back">
            <form name="enterInUserRoom" id="showTaxiForm" action="${request.getContextPath}/project/controller" method="post" >
              <a class="navbar-brand" href="javascript:;" onclick="parentNode.submit();">
                   ...
                   <input type="hidden" name="page" value="${previoussPage}"/>
...
<div class="container">
  <div class="row">
    ...
    <div class="col" id="specialTable">
                              ...
                              <h4><fmt:message key="tripCalculation" bundle="${rb}"/></h4>
...
<br/>
    ...
    <table class="table table-bordered table-hover table-secondary text-white bg-secondary" id="tableCalcul">
         <caption><h6> ${discountMessage} ${discount}</h6></caption>
      <thead>
        <tr>
          <th scope="col" class="table-active"><fmt:message key="timeTaxi" bundle="${ rb }"/></th>
          <th scope="col" class="table-active"><fmt:message key="lengthOfRoute" bundle="${ rb }"/></th>
          <th scope="col" class="table-active"><fmt:message key="timeOfRoute" bundle="${ rb }"/></th>
          <th scope="col" class="table-active"><fmt:message key="costOfRoute" bundle="${ rb }"/></th>
      ...
      <tbody class="table-hover">
        ...
        <tr>
          ...
          <td><c:out value="${routeDistance}"/> <fmt:message key="km" bundle="${rb}"/><br/></td>
          <td><c:out value="${routeTime}"/> <fmt:message key="min" bundle="${rb}"/></td>
          <td><c:out value="${routeCost}"/> <fmt:message key="rub" bundle="${rb}"/></td>
        </tr>
